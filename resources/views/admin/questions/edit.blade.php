<x-app-layout>
    ...
    <div class="py-10 max-w-7xl mx-auto sm:px-6 lg:px-8">
        {{-- PENTING: Action mengarah ke UPDATE dan punya @method('PUT') --}}
        <form action="{{ route('admin.exams.questions.update', [$exam->id, $question->id]) }}" method="POST"
            ...
            @php
            // Ekstrak data lama dari database untuk dilempar ke AlpineJS
            $oldOptions = $question->type === 'matching' ? $question->matches : $question->options;
            @endphp

            <div x-data="optionManager('{{ $question->type }}', {{ json_encode($oldOptions) }})"
                ...
                <div class="lg:col-span-7 space-y-6">
                    <div class="bg-white p-6 rounded-3xl shadow-sm border border-slate-200">
                        ...
                        <div wire:ignore>
                            <textarea id="editor_content" name="content">{!! $question->content !!}</textarea>
                        </div>
                    </div>
                </div>
                ...
                <div class="lg:col-span-5">
                                    ...
                                    <div class="flex-1 space-y-2">
                                        ...
                                        <template x-if="type !== 'matching'">
                                            <textarea x-model="opt.option_text"
                                                ...
                                                class="w-full text-sm border-slate-200 rounded-lg" rows="2"></textarea>
                                        </template>
                                    </div>
                    ...
                    <div class="mt-6 flex justify-end gap-3">
                        ...
                        <button type="submit"
                            class="px-8 py-3 bg-indigo-600 text-white font-bold rounded-xl shadow-lg hover:bg-indigo-700">Update
                            Soal</button>
                    </div>
                </div>
            </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            const editor = SUNEDITOR.create('editor_content', {
                ...
                buttonList: [['undo', 'redo'], ['bold', 'underline', 'italic'], ['align', 'list', 'table'], ['image', 'math'], ['fullScreen']]
            });
        ...
        document.addEventListener('alpine:init', () => {
            // Menerima data bawaan dari database
            Alpine.data('optionManager', (initialType, initialOptions = []) => ({
                type: initialType,
                options: initialOptions,

                init() {
                    // Jika data kosong (gagal diload/tidak ada), buat baris default
                    if(this.options.length === 0) this.resetOptions();
                },

                resetOptions() { /* Sama seperti file create.blade.php */
                    this.options = [];
                ...
                addOption() { /* Sama seperti file create.blade.php */
