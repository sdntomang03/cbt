<x-cbt-layout>
        ...
        <div class="flex gap-4">
            <button onclick="location.reload()"
                class="px-8 py-3.5 rounded-xl font-bold bg-emerald-500 text-white hover:bg-emerald-600 shadow-lg transition hover:scale-105"><i
                    class="fas fa-sync-alt mr-2"></i> Muat Ulang</button>
    ...
    <div
        ...
        <h1 class="text-4xl font-black mb-4 uppercase tracking-wider">UJIAN SELESAI</h1>
        ...
        <div class="flex gap-4">
            <a href="{{ route('public.exams.result', $exam) }}"
                class="px-8 py-3.5 rounded-xl font-bold bg-emerald-500 text-white hover:bg-emerald-600 shadow-lg transition hover:scale-105"><i
                    class="fas fa-chart-bar mr-2"></i> Lihat Nilai</a>
        </div>
    </div>
    @else
    ...
    <div class="fixed inset-0 flex flex-col bg-[#f1f5f9]" x-data='examRunner(
        ...
        <div
            ...
            <div class="bg-slate-900 text-white px-3 py-1.5 lg:px-5 lg:py-2 rounded-xl font-mono font-bold text-sm lg:text-lg flex items-center gap-1.5 lg:gap-2 shadow-lg"
                ...
                <i class="fas fa-clock text-xs opacity-50"></i> <span x-text="formatTime(timeLeft)"></span>
            </div>
            ...
            <div class="flex items-center gap-2">
                <button type="button" onclick="toggleMobileNav()" id="mobile-nav-btn"
                    ...
                    <i class="fas fa-th text-sm"></i> <span class="text-xs font-black">Soal</span>
                </button>
        ...
        <div class="flex-1 flex overflow-hidden">
                ...
                <div class="max-w-4xl mx-auto min-h-[400px] relative">
                    ...
                    <div x-show="isLoading"
                        ...
                        <div class="flex flex-col items-center gap-3">
                            <i class="fas fa-spinner fa-spin text-3xl text-indigo-600"></i> <span
                                class="font-bold text-slate-500 text-sm">Memuat soal...</span>
                        </div>
                    </div>
                        ...
                        <div x-transition:enter="transition duration-300 ease-out"
                            ...
                            <div :style="`zoom: ${zoomLevel};`" class="origin-top">
                                ...
                                <div class="space-y-4">
                                    <template x-if="q.type === 'single_choice' || q.type === 'true_false'">
                                        <div class="space-y-3">
                                            <template x-for="opt in q.options" :key="opt.id">
                                                <label
                                                    class="flex items-start gap-4 p-4 border rounded-2xl cursor-pointer transition-all duration-200"
                                                    :class="answers[q.id] == opt.id ? 'bg-indigo-50 border-indigo-500 shadow-sm ring-1 ring-indigo-500' : 'bg-white border-slate-200 hover:bg-slate-50'">
                                                    <div class="flex-shrink-0 mt-0.5">
                                                        <input type="radio" :name="'answer_' + q.id" :value="opt.id"
                                                            x-model="answers[q.id]" @change="saveAnswer(q.id, opt.id)"
                                    ...
                                    <template x-if="q.type === 'complex_choice'">
                                        <div class="space-y-3">
                                            <template x-for="opt in q.options" :key="opt.id">
                                                <label
                                                    class="flex items-start gap-4 p-4 border rounded-2xl cursor-pointer transition-all duration-200"
                                                    :class="isOptionSelected(q.id, opt.id) ? 'bg-indigo-50 border-indigo-500 shadow-sm ring-1 ring-indigo-500' : 'bg-white border-slate-200 hover:bg-slate-50'">
                                                    <div class="flex-shrink-0 mt-0.5">
                                                        <input type="checkbox" :value="opt.id"
                                                            :checked="isOptionSelected(q.id, opt.id)"
                                                            @change="toggleMultipleAnswer(q.id, opt.id)"
                                    ...
                                    <template x-if="q.type === 'essay'">
                                        <textarea x-model="answers[q.id]" @blur="saveAnswer(q.id, answers[q.id])"
                                            class="w-full min-h-[150px] p-4 rounded-xl border-slate-200 focus:border-indigo-500 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 text-slate-700"
            ...
            <div
                ...
                <div class="p-5 bg-white border-b border-slate-100">
                    ...
                    <p class="text-xs text-slate-400 mt-0.5">Terjawab: <span class="font-bold text-indigo-600"
                        ...
                        <template x-for="(qId, index) in questionIds" :key="qId">
                            <button @click="gotoQuestion(index)"
                                ...
                                :class="{
                                        ...
                                        'bg-emerald-500 text-white border-emerald-500': hasAnswer(qId) && !flags.includes(qId) && currentIndex !== index,
                                        'bg-white text-slate-400 border-slate-200 hover:border-indigo-300': !hasAnswer(qId) && !flags.includes(qId) && currentIndex !== index
                ...
                <div class="p-4 border-t border-slate-100 bg-white space-y-1.5">
                    ...
                    <div class="flex items-center gap-2 text-xs text-slate-600">
                        <div class="w-4 h-4 rounded bg-emerald-500 shrink-0"></div><span>Sudah dijawab</span>
                    </div>
                    <div class="flex items-center gap-2 text-xs text-slate-600">
        ...
        <div
            class="shrink-0 h-16 bg-white/80 backdrop-blur border-t border-slate-100 flex items-center justify-between px-4 sm:px-8 z-[100] shadow-[0_-4px_20px_rgba(0,0,0,0.04)]">
            ...
            <div class="flex gap-2">
                <button x-show="currentIndex < questionIds.length - 1" @click="nextQuestion()"
                    class="flex items-center gap-2 px-5 py-2 rounded-xl font-bold text-sm bg-indigo-600 hover:bg-indigo-700 text-white transition-all shadow-md hover:-translate-y-0.5">
                    Selanjutnya <i class="fas fa-chevron-right text-xs"></i>
                </button>
                <button x-show="currentIndex === questionIds.length - 1" @click="finishExam()" x-cloak
                    class="flex items-center gap-2 px-5 py-2 rounded-xl font-bold text-sm bg-emerald-500 hover:bg-emerald-600 text-white transition-all shadow-md hover:-translate-y-0.5 animate-pulse">
                    Selesai <i class="fas fa-check-double text-xs"></i>
                </button>
            </div>
        </div>
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        wrap.addEventListener('touchend', e => {
            ...
            function toggleMobileNav() {
                ...
                if (panel.classList.contains('open')) { panel.classList.remove('open'); overlay.classList.remove('open'); }
                else { renderMobileNav(); panel.classList.add('open'); overlay.classList.add('open'); }
            }

            function renderMobileNav() {
                const runner = window._examRunner, grid = document.getElementById('mobile-nav-grid');
                if (!runner || !grid) return;
                grid.innerHTML = '';
                runner.questionIds.forEach((qId, index) => {
                    const btn = document.createElement('button');
                    btn.textContent = index + 1;
                    const isCurrent = runner.currentIndex === index, isFlagged = runner.flags.includes(qId), isAnswered = runner.hasAnswer(qId);
                    ...
                    else if (isAnswered) cls += 'bg-emerald-500 text-white border-emerald-500';
